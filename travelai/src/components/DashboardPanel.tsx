...
export const DashboardPanel: React.FC<DashboardPanelProps> = ({
  ...
  return (
    <section
      ...
      className="w-full xl:w-[54%] flex flex-col rounded-xl bg-surface-container-low/70 backdrop-blur-xl border border-surface-container-high/40 shadow-xl overflow-hidden min-w-0 h-full max-h-[calc(100vh-6rem)] relative"
    >
      {/* Toast Notification */}
      {toastMessage && (
      ...
      {/* Dashboard Header */}
      <div className="p-space-md bg-surface-container/90 border-b border-surface-container-high/40 flex flex-col sm:flex-row sm:items-center justify-between gap-space-md">
        <div className="flex flex-col min-w-0">
          <div className="flex items-center gap-2">
            <h1 className="text-base sm:text-lg lg:text-xl font-semibold text-on-surface truncate">
              {trip?.title || 'Goa Coastal Splendor & Heritage'}
            </h1>
            <span className="px-2 py-0.5 rounded-full bg-tertiary/10 text-tertiary text-[11px] font-semibold whitespace-nowrap border border-tertiary/20">
      ...
      <div className="flex-1 overflow-y-auto p-space-md lg:p-space-lg space-y-space-md">
        ...
        {activeTab === 'flights-tab' && (
          <FlightsTab flights={flights} onSelectFlight={onSelectFlight} />
        )}
        {activeTab === 'hotels-tab' && (
          <HotelsTab hotels={hotels} onSelectHotel={onSelectHotel} />
        )}
        {activeTab === 'weather-tab' && <WeatherTab weather={weather} />}
