...
import { ChatMessage, ApiStatusState } from '../types';
...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  const quickPrompts = [
    { label: '✨ Plan 3 days in Jaipur', text: 'Plan 3 days in Jaipur with heritage Havelis' },
    { label: '🏖️ Best time to visit Goa?', text: 'What is the best time to visit Goa for calm waters?' },
    { label: '🏙️ 5-star hotel in Mumbai', text: 'Find 5-star luxury hotel in Mumbai near Marine Drive' },
    { label: '💳 Budget breakdown under ₹30,000', text: 'Provide a granular budget breakdown under ₹30,000' },
  ];
  ...
  return (
    <section
      ...
      className="w-full xl:w-[46%] flex flex-col rounded-xl bg-surface-container-low/70 backdrop-blur-xl border border-surface-container-high/40 shadow-xl overflow-hidden min-w-0 h-full max-h-[calc(100vh-6rem)]"
    >
      {/* Live Context Status Bar */}
      <div className="p-space-md bg-surface-container/90 border-b border-surface-container-high/40 flex flex-col gap-space-sm">
        ...
        <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
          <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-surface-container-high text-on-surface shadow-sm border border-surface-container-highest/60 text-xs">
          ...
          <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-surface-container-high text-on-surface shadow-sm border border-surface-container-highest/60 text-xs">
            ...
            <span>₹50,000 • Luxury Leisure</span>
          </div>
          <div className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-surface-container-highest text-on-surface-variant text-xs">
      ...
      <div className="px-space-md py-1.5 bg-surface-container-lowest/50 border-t border-surface-container-high/30 flex items-center gap-1.5 overflow-x-auto no-scrollbar">
        {quickPrompts.map((chip, idx) => (
          <button
            ...
            className="whitespace-nowrap px-2.5 py-1 rounded-full bg-surface-container-high hover:bg-surface-bright text-on-surface-variant hover:text-on-surface text-xs transition-colors border border-surface-container-highest/60 flex-shrink-0"
          >
            {chip.label}
          </button>
        ))}
